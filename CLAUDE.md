...
3. **The honest log.** The scrollback is a truthful, reproducible history. Any action taken by a widget or the assistant that changes the user's shell, filesystem, or repository happens by emitting a real, visible command, never a hidden side effect. Changes to Shax's own state, and OS-level assertions Shax holds on its own behalf, are not commands and are not logged — but must carry a permanently visible readout in the UI. Where it is unclear which side an action falls on, it goes through the emit path: the default is disclosure.
