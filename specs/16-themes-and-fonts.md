...
- A theme carries its own **inks for coloured surfaces** — `chrome.accent-fg`, `chrome.green-fg`, `chrome.red-fg` — the text colour used wherever that colour is a *background*: `--accent` for the statusline mode pill, the calendar's today chip and every primary button; `--green` for the assistant's read-only Run probe; `--red` for the safety gate's "Run anyway" and the assistant's destructive proposal. They are deliberately not `--fg`, `--bg`, or a literal white, each of which is readable only on presets where that surface happens to contrast with it. Every one must reach WCAG AA (4.5:1) against its own surface, which `themes.rs` enforces as a test — a new preset cannot ship an illegible chip.
- The three inks are **separate tokens, not one reused**, because the right ink is per-surface as well as per-theme: five presets want a *light* ink on `--red` (Solarized's red is dark enough that white wins) while wanting a dark one on `--accent`. Reusing `accent-fg` everywhere would regress Nord's red from 4.09:1 to 3.56:1.
- A veil painted *on top of* one of these surfaces — the `⌥⏎` mnemonic chip on an approve button — derives from `currentColor` rather than a fixed translucent white, so it darkens or lightens to match whichever ink the button is using.
