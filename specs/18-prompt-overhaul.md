...
**Scope:** extend the mirror renderer and the strip DOM to N rows; add the Shift+Enter binding and paste safety.

- `promptRenderer.ts` grows from a single-row model to a `rows: PromptRow[]` model. `\n` in the incoming stream appends a new row instead of clearing the current one. Cursor position becomes `(row, col)`. Style-tracking (`styled` bit per cell) survives per-row.
- `PromptStrip.tsx` renders as a flex column of rows; the cursor bar spans exactly one row-height. The strip's outer flex grows vertically as rows accumulate; the pane's block list yields the space (existing flex layout already does this correctly for the empty-state hero — verify).
- `keyToBytes.ts` gains a Shift+Enter mapping to `\\\n` (backslash + newline). Enter continues to send `\n`.
- Paste handler (`PromptStrip.tsx:219-234`) is rewritten:
  - Bracketed-paste wrap (`\e[200~ … \e[201~`) always, unless the shell has explicitly disabled it (rare; we do not detect this in v1).
  - If the paste is ≥ 5 lines OR ≥ 500 bytes, open a new `ConfirmPasteModal` (register in the modal layer, dismissable with Esc). The modal shows the payload, a "Paste as one command" toggle default-on, and Paste / Cancel buttons. On Paste, the bytes go through as bracketed paste; the shell interprets embedded newlines as line-ends if the toggle is off, or as literal newlines if on (achieved by prepending `\` to each embedded LF client-side).
- The B-marker-in-PROMPT hack in `shax.zsh:56-58` becomes unnecessary in assertive mode (PS1 is already bare A+B); leave the append conditional on `SHAX_SHELL_INTEGRATION_MODE=cooperative` for backward compatibility.

**Exit:** typing `echo "hello` and pressing Enter drops to PS2 with the second row visible in the strip (mirrored from the shell's echo); typing `echo hello \` and Shift+Enter puts a visible newline in the strip and drops to PS2; a 200-line paste opens the confirmation modal.
