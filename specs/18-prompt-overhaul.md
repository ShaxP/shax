...
**Exit:** typing `echo "hello` and pressing Enter drops to PS2 with the second row visible in the strip (mirrored from the shell's echo); typing `echo hello` and pressing Shift+Enter puts a visible newline in the strip and drops to PS2; pasting a 6-line snippet opens the confirmation modal, and after Paste the strip shows the payload as a multi-line buffer that executes as separate commands when the user presses Enter — no folding, no repetition on redraw.
