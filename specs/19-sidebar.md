...
2. **CPU / Memory** — two cards. Memory is a donut + `used / total`. CPU is a header percentage over a 24-bar sparkline of recent samples, with a `load N.NN` / `N cores` footer (load average omitted on Windows, which has none — `sysinfo` returns zeros there rather than failing, and printing `load 0.00` would be a fabricated reading). Each bar is coloured by *its own* load — green below 50, amber below 80, red at or above it — so a red stretch behind a green newest bar says "it spiked and recovered"; recency is carried by opacity, keeping hue and brightness independent. Uses `sysinfo` v0.32 (cross-platform, actively maintained, one crate for all the signals). **Deferred out of scope:** per-core breakdown, top-N processes, swap.
   ...
   **Amended (M13.5).** The refreshed `design/sidebar-extended.png` mockup drops the per-bar heat colouring: historical bars render in a monochrome `--fg-faint` with a left-to-right opacity gradient (0.4 at the oldest to 0.9 at the newest historical), and only the newest bar itself carries the heat colour. That trades the "red stretch behind a green newest bar says it spiked and recovered" magnitude signal — history is now shape-only (activity vs quiet), magnitude comes from the current bar plus the header percent, and the gradient carries recency alone. Green/amber/red thresholds and the `heatColour` helper survive unchanged, still governing the newest bar and the header percent (M13.5.2).
...
- **Card border becomes accent-blue AND the interior fills with `--accent-soft`** (a ~14 % accent, the same soft-fill token the assistant dock uses for selected messages) when the assertion is held. Same border weight as the resting card; only the colour changes there. Border + soft fill together are what make on/off legible — border alone reads as "another card with a coloured border" against the surrounding sidebar, where every widget already carries a border. The interior tint is what makes the active card visibly a *different* thing.
...
4. **CPU** — historical bars flatten to a monochrome `--fg-faint` with a left-to-right opacity gradient (oldest dim, newest historical bright) per `design/sidebar-extended.png`; the newest bar continues to carry its heat colour, and the header percent continues to read from the same `heatColour` helper. Everything else about the CPU card (24-bar sparkline, `load N.NN · N cores` footer, the M13-refinement extended card format) is unchanged. See D5 item 2's M13.5 amendment for the reasoning.
