...
3. **Network** — SSID + default-route IP + up/down dot, plus `↑`/`↓` throughput lines (green out, cyan in). **The mockup and this spec describe different cards here:** `design/widget-sidebar.png` shows a *half-width* Network card carrying throughput **instead of** identity, paired with a Disk card Phase 1 doesn't build. We keep the full width and add the rates beneath the IP, so the M13.3 SSID probe isn't discarded to gain them — and no lonely half-width card sits beside a hole. Throughput rides the 2s system sampler rather than this widget's 30s poll, because it is a delta whose meaning depends on the interval it was measured over (same reason CPU moved there). It is scoped to the interface holding the default-route IP rather than summed across all of them: summing counts VPN and Docker adapters, so a `docker pull` would report roughly double what crossed the wire. Both directions render together or not at all — one arrow alone reads as "the other direction is idle" rather than "unmeasured". Local IP reuses the M12.4b `system_local_ip()` command. SSID and medium come from `wifi_info()`: Linux `iwgetid -r` + `/sys/class/net/<iface>/wireless`, Windows `netsh wlan show interfaces`, macOS CoreWLAN + `networksetup -listallhardwareports`.

   **Amended (M13 refinement): macOS requests location access for the SSID, and medium is probed separately.** M13.3 returned `None` for the macOS SSID rather than request an entitlement, which was the right call in isolation. The card then rendered the missing name as `Wired` — turning the *absence* of one fact into a positive claim of a different one. Since macOS never returns an SSID without authorisation, every Wi-Fi Mac was told it was on Ethernet: not a missing reading but a wrong one, and a fidelity-contract violation (#2). A test asserted the behaviour, so it codified the defect instead of catching it.

   Two independent fixes. **The medium is now probed on its own** and needs no permission anywhere, so a nameless network is still known to be wireless; an unrecognised port or interface reports `unknown`, never `wired`. **The SSID on macOS now asks**, via `NSLocationWhenInUseUsageDescription` and `CLLocationManager.requestWhenInUseAuthorization`, because every non-authorised path is redacted — verified on macOS 26.3, where `networksetup -getairportnetwork` claims you aren't associated with any network at all. Shax never reads, stores, or transmits a location; the authorisation exists solely so the OS will name the network you are already on.

   **The prompt is offered, never forced.** A permission dialog at launch, before the user has asked for anything, is the intrusion non-negotiable #1 rules out. The card shows a "Show network name…" affordance only when it would change something — wireless, online, and prompt unanswered — and declining costs the name and nothing else. Up/down is presence-of-default-route (already computable from the local-IP probe returning `Some`), no ping — a widget that pings on a poll would be a network chatter source in a "local-first, no telemetry" product. Refresh cadence 30s, matching battery and local-IP.
