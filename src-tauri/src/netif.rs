...
mod imp {
    ...
    /// Windows names interfaces descriptively ("Wi-Fi", "Ethernet"),
    /// so the name carries the answer that `networksetup` gives on
    /// macOS. Anything unrecognised stays `Other` rather than being
    /// guessed at.
    pub fn classify(interface: &str) -> InterfaceKind {
        let lower = interface.to_ascii_lowercase();
        if lower.contains("wi-fi") || lower.contains("wifi") || lower.contains("wireless") {
            InterfaceKind::WiFi
        } else if lower.contains("ethernet") || lower.contains("local area connection") {
